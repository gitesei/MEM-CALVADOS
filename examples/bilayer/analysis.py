...
def calc_bilayer_prop(path,output_prefix,saving_interval,nskip,cooke_model=False):
    ...
    """
    # center trajectory based on tail beads
    h, z = center_slab(path,path=path,ref_atoms='resname LTL')
    # load centered trajectory
    traj = md.load_dcd(path+'/traj.dcd',top=path+'/top.pdb')[nskip:]
    h = h[nskip:]
    # simulation time
    ...
    t_head = traj.atom_slice(traj.top.select('resname LHD'))
    ...
    if not cooke_model:
        ...
        t_pho = traj.atom_slice(traj.top.select('resname LHD'))
    ...
    protein_selection = traj.top.select('all and not (resname LHD or resname LTL)')
    ...
    if cooke_model:
        last_tail = traj.top.select('resname LTL')[1::2]
    else:
        last_tail = traj.top.select('resname LTL')[2::3]

    head = traj.top.select('resname LHD')
...
def calc_tmd_prop(path,output_prefix,saving_interval,nskip,tmd):
    ...
    # load centered trajectory
    traj = md.load_dcd(path+'/traj.dcd',top=path+'/top.pdb')[nskip:]
    traj = traj.atom_slice(traj.top.select('all and not (resname LHD or resname LTL)'))
    ...
    for chainid in range(traj.n_chains):
        ...
        for residue in atom_slice.top.residues:
            res = top.add_residue(residue.name, chain, resSeq=resnum)
            resnum += 1
